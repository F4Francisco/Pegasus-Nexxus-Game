...
public class SmallPowerUp extends Entity {
	...
	public SmallPowerUp(int x, int y) {
		...
		if (x <= (Game.getInstance().getWidth() / 7)) {
			direction = -30;
		} else if (x >= Game.getInstance().getWidth()
				- (Game.getInstance().getWidth() / 5)) {
			direction = 30;
		} else {
			if (Math.random() >= 0.5) {
				direction = -30;
	...
	public void move(long delta) {
		...
		if (Game.getInstance().getCharacter().getInvincibility()
				- Game.getInstance().getGameTime() - 2000 > 0) {
			collected = 0;
		}
		if (collected == 2) {
			playerDirection = Math2D.direction(new Point(x, y), new Point(Game
					.getInstance().getHitBox().getX(), Game.getInstance()
					.getHitBox().getY()));
			...
			if (Game.getInstance().getHitBox().getY() <= Game.getInstance()
					.getHeight() / 5
			...
			if ((x - Game.getInstance().getHitBox().getX() <= 40 && x
					- Game.getInstance().getHitBox().getX() >= -40)
					&& (y - Game.getInstance().getHitBox().getY() <= 40 && y
							- Game.getInstance().getHitBox().getY() >= -40)) {
				collected = 1;
			}
	...
	public void collidedWith(Entity other) {
		if (other instanceof CharHitBox) {
			if (Game.getInstance().getGameTime() > Game.getInstance()
					.getCharacter().getInvincibility() - 2000) {
				if (startTime < Game.getInstance().getGameTime() - 200) {
					...
					new AePlayWave(Game.getInstance().getPath()
							+ "/src/sounds/se_item00.wav").start();
					Game.getInstance().removeEntity(this);
				}
			}
		}
